        ...
        <tbody>
            @foreach ($fees as $fee)
                <tr>
                    ...
                    <td>{{ $fee->course->name_of_course ?? 'N/A' }}</td>
                    <td>{{ $fee->course_fee }}</td>
                    <td>{{ $fee->course->duration }} Days</td>
                    <td>
                        @if ($fee->payment_details)
                            <table class="table table-sm table-bordered">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Payment Date</th>
                                        <th>Status</th>
                                        @can('add student fee transaction')
                                            <th>Action</th> <!-- Only show if user has permission -->
                                        @endcan
                                    </tr>
                                </thead>
                                <tbody>
                                            ...
                                            <td>
                                                ...
                                                @else
                                                    <span style="color: red; font-weight: bold;">Pending
                                                        @php
                                                            $daysPending = \Carbon\Carbon::parse($payment->payment_date)->diffInDays(\Carbon\Carbon::now());
                                                            $daysPending = max(0, (int) $daysPending);
                                                        @endphp
                                                        @if ($daysPending != 0)
                                                            <br>({{ $daysPending }})
                                                        @endif
